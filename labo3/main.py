...
def proyectarPtsAfin(T, wz):
    assert(T.shape == (3,3)) 
    fila_unos = np.ones((1, wz.shape[1]))
    nuevoWZ =  np.vstack((wz, fila_unos))
    xy = T @ np.array(nuevoWZ)
    return xy[:2]
...
def vistform(T, wz, titulo=''):
    ...
    #xy = proyectarPtsAfin(T, wz) agrega las traslaciones
    if xy is None:
